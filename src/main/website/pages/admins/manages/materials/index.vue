...
import {createOrUpdateMaterial, getAllMaterial, getMaterialById} from '~/server/services/material'
...
let state = reactive(new Material)
...
<template>
  <UDashboardPage>
    <UDashboardPanel grow>
      <UDashboardNavbar title="Chất liệu" :badge="totalRows">
        <template #right>
          <UInput
            ref="input"
      ...
      <UDashboardModal
        ...
        :title="state.id ? 'Cập nhật Chất liệu' : 'Thêm mới Chất liệu'"
      ...
      >
        ...
        >
          <UFormGroup label="Tên Chất liệu" name="name">
            <UInput v-model="state.name" placeholder="Cotton, kaki,..." autofocus/>
          </UFormGroup>

          <UFormGroup label="Mô tả" name="description">
            <UInput v-model="state.description" placeholder="Cotton, kaki,..."/>
          </UFormGroup>
