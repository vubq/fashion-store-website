<template>
  ...
  >
    <component :is="draggable && Draggable ? Draggable : 'li'" v-for="(link, index) in links" :key="index" tag="li" :class="ui.container">
      <Disclosure v-slot="{ open }" :default-open="link.defaultOpen === undefined ? true : link.defaultOpen" as="div">
        <DisclosureButton as="template">
          ...
          >
            ...
            >
              ...
              <UIcon v-if="link.children?.length && link.collapsible !== false" :name="ui.trailingIcon.name" :class="[ui.trailingIcon.base, open ? ui.trailingIcon.active : ui.trailingIcon.inactive]" />
        ...
        <Transition
            ...
            @leave="onLeave"
        >
          <DisclosurePanel v-if="link.children?.length && (open || link.collapsible === false)" static as="template">
...
const config = computed(() => ({
  ...
  active: 'text-black dark:text-white before:bg-gray-100 dark:before:bg-gray-800',
  inactive: 'text-black dark:text-gray-400 hover:text-gray-900 dark:hover:text-white hover:before:bg-gray-50 dark:hover:before:bg-gray-800/50',
  static: 'text-black dark:text-white cursor-auto',
  icon: {
    base: 'flex-shrink-0 w-5 h-5',
    active: 'text-black dark:text-white',
    inactive: 'text-black dark:text-gray-500 group-hover:text-gray-700 dark:group-hover:text-gray-200'
  },
  trailingIcon: {
